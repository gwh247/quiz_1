...
# iterate through the csv object   id,first_name,last_name,gender,email,phone
for record in csvfile: 
    full_name = int(record[1]) + " " + int(record[2])
    email = int(record[4])
    phone_num = int(record[5])
   



    # add the key-value pair to the dictionary
    dict1['full_name'] = [full_name,email, phone_num]
